Feature: Create a wp-config file
  ...
  Scenario: Configure with required SSL connection
    Given an empty directory
    And WP files
    And I run `MYSQL_PWD='{DB_ROOT_PASSWORD}' mysql -u root -h{MYSQL_HOST} -P{MYSQL_PORT} -e "CREATE USER IF NOT EXISTS 'wp_cli_test_ssl'@'127.0.0.1' IDENTIFIED BY 'password2' REQUIRE SSL;"`
